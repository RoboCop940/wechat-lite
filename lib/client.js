...
WeChatClient.prototype.status = function(uuid){
  return this.request('get', WeChatClient.API_LOGIN + '/cgi-bin/mmwebwx-bin/login', {
    ...
    return WeChatClient.parseJS(code);
  }, true);
};
 ...
 * @return {[type]}      [description]
 */
WeChatClient.prototype.init = function (data) {
  return this.request('post',  WeChatClient.API_LOGIN + '/cgi-bin/mmwebwx-bin/webwxinit', null, {
    BaseRequest: this.BaseRequest(data)
  }, null, null, { Cookie: this.cookie(data) });
};
...
WeChatClient.prototype.check = function(data, synckey){
  const query = Object.assign(lowercase(this.BaseRequest(data)), {
    synckey: synckey.List.map(function(item){
      return [ item.Key, item.Val ].join('_')
    }).join('|')
  });
  return this.request('get', 'https://webpush.weixin.qq.com/cgi-bin/mmwebwx-bin/synccheck', query, {})
  .then(function(res){
...
WeChatClient.prototype.loop = function({ User, SyncKey }){
  return this.check(User, SyncKey);
};
