'use strict';
const util   = require('util');
const WeChat = require('.');
/**
 * [WeChatClient description]
 * @param {[type]} api [description]
 */
function WeChatClient(api){
  WeChat.apply(this, arguments);
  this.DeviceID = `e${+new Date}`;
  return this;
};

/**
 * [EVENTS description]
 * @type {Object}
 */
WeChatClient.EVENTS = {
  SCAN      : 'scan',
  LOGIN     : 'login',
  READY     : 'ready',
  MESSAGE   : 'message',
  CONTACTS  : 'contacts',
};
/**
 * [VERIFY_FLAG description]
 * @type {Object}
 */
WeChatClient.VERIFY_FLAG = {
  USER: 0,
  OFFICIAL: 24,
  GUANFANG: 56
};
/**
 * [MSG_TYPE description]
 * @type {Object}
 */
WeChatClient.MSG_TYPE = {
  TEXT: 1,
  IMAGE: 3,
  VOICE: 34,
  VIDEO: 43,
  MICRO_VIDEO: 62,
  EMOTICON: 47,
  APP: 49,
  VOIP_MSG: 50,
  VOIP_NOTIFY: 52,
  VOIP_INVITE: 53,
  LOCATION: 48,
  STATUS_NOTIFY: 51,
  SYSNOTICE: 9999,
  POSSIBLE_FRIEND_MSG: 40,
  VERIFY_MSG: 37,
  SHARE_CARD: 42,
  SYS     : 10000,
  RECALLED: 10002,
};
/**
 * [CONTACT_FLAG description]
 * @type {Object}
 */
WeChatClient.CONTACT_FLAG = {
  CONTACT: 1,
  CHATCONTACT: 2, 
  SUBSCRIBE: 3,
  CHATROOMCONTACT: 4,
  BLACKLISTCONTACT: 8,
  DOMAINCONTACT: 16,
  HIDECONTACT: 32,
  FAVOURCONTACT: 64,
  SNSBLACKLISTCONTACT: 256,
  NOTIFYCLOSECONTACT: 512,
  TOPCONTACT: 2048,
};
/**
 * [PROFILE_BITFLAG description]
 * @type {Object}
 */
WeChatClient.PROFILE_BITFLAG = {
  NOCHANGE: 0,
  CHANGE: 190,
};
/**
 * [STATUS_NOTIFY_CODE description]
 * @type {Object}
 */
WeChatClient.STATUS_NOTIFY_CODE = {
  READED: 1,
  ENTER_SESSION: 2,
  INITED: 3,
  SYNC_CONV: 4,
  QUIT_SESSION: 5,
  MOMENTS: 9
};
/**
 * [CHAT_ROOM_NOTIFY description]
 * @type {Object}
 */
WeChatClient.CHAT_ROOM_NOTIFY = {
  OPEN: 1,
  CLOSE: 0,
};
/**
 * [SEX description]
 * @type {Object}
 */
WeChatClient.SEX = {
  MALE: 1,
  FEMALE: 2,
};
/**
 * [inherits description]
 * @param  {[type]} WeChatClient [description]
 * @param  {[type]} WeChat       [description]
 * @return {[type]}              [description]
 */
util.inherits(WeChatClient, WeChat);
/**
 * [API_LOGIN description]
 * @type {String}
 */
WeChatClient.API_LOGIN = 'https://login.weixin.qq.com';

/**
 * [parseJS description]
 * @param  {[type]} code  [description]
 * @param  {[type]} scope [description]
 * @return {[type]}       [description]
 */
WeChatClient.parseJS = function(code, scope){
  var window = {};
  if(scope){
    window[ scope ] = {};
  }
  eval(code);
  return scope ? window[scope] : window;
};

/**
 * [getUUID description]
 * @return {[type]} [description]
 */
WeChatClient.prototype.uuid = function(){
  return this.request(WeChatClient.API_LOGIN + '/jslogin', {
    query: { appid: this.options.appId },
  })
  .then(res => res.text())
  .then(res => {
    return WeChatClient.parseJS(res, 'QRLogin').uuid
  });
};

/**
 * [qrcode description]
 * @param  {[type]} uuid [description]
 * @return {[type]}      [description]
 */
WeChatClient.prototype.qrcode = function(uuid){
  return [ WeChatClient.API_LOGIN, 'qrcode', uuid ].join('/');
};

/***/
WeChatClient.prototype.printQrcode = function (uuid) {
  console.log('[WeChat]> %s', this.qrcode(uuid));
  return uuid;
};

WeChatClient.prototype.wait = function(uuid){
  var self = this;
  return new Promise(function(accept, reject){
    (function loop(){
      self.status(uuid).then(function(status){
        switch (status.code) {
          case 200:
            self.emit(WeChatClient.EVENTS.LOGIN, status);
            accept(status.redirect_uri);
            break;
          case 201:
            self.emit(WeChatClient.EVENTS.SCAN, status);
            setTimeout(loop, 1000);
            break;
          case 408:
            // 未确认（显示二维码后30秒触发）
          default:
            console.log('status', status);
            reject(status);
            break;
        }
      });
    })();
  });
};

/**
 * [status description]
 * @param  {[type]} uuid [description]
 * @return {[type]}      [description]
 */
WeChatClient.prototype.status = function(uuid){
  return this.request(WeChatClient.API_LOGIN + '/cgi-bin/mmwebwx-bin/login', {
    query: { uuid }
  })
  .then(res => res.text())
  .then(WeChatClient.parseJS);
};

/**
 * [login description]
 * @param  {[type]} uuid   [description]
 * @param  {[type]} ticket [description]
 * @return {[type]}        [description]
 */
WeChatClient.prototype.login = function(url){
  return this.request(url).then(({ headers }) => {
    var data = { };
    headers['set-cookie'].filter(function(cookie){
      return /wxuin|wxsid|webwx_data_ticket/.test(cookie);
    }).map(function(cookie){
      return cookie.split(';')[0].split('=');
    }).forEach(function(item){
      data[ item[0] ] = item[1];
    });
    return data;
  });
};
/**
 * @param {[type]} data [description]
 */
WeChatClient.prototype.BaseRequest = function(data){
  return {
    Uin      : data.wxuin,
    Sid      : data.wxsid,
    Skey     : '' ,
    DeviceID : this.deviceId
  };
};
/**
 * @param  {[type]} data [description]
 * @return {[type]}      [description]
 */
WeChatClient.prototype.cookie = function(data){
  return Object.keys(data).map(function(key){
    return [ key, data[ key ] ].join('=');
  }).join('; ');
};
/**
 * @param  {[type]} data [description]
 * @return {[type]}      [description]
 */
WeChatClient.prototype.init = function (session) {
  return this.request(WeChatClient.API_LOGIN + '/cgi-bin/mmwebwx-bin/webwxinit', {
    method: 'post',
    headers: { Cookie: this.cookie(session) },
    body: { BaseRequest: this.BaseRequest(session) },
  }).then(res => res.json())
};

/**
 * [send description]
 * @return {[type]} [description]
 */
WeChatClient.prototype.send = function () {
  
};

function lowercase(d){
  var o = {};
  for(var k in d) o[ k.toLowerCase() ] = d[k];
  return o;
}

WeChatClient.prototype.check = function(session, synckey){
  this.SyncKey = synckey;
  const query = Object.assign(lowercase(this.BaseRequest(session)), {
    synckey: synckey.List.map(function(item){
      return [ item.Key, item.Val ].join('_')
    }).join('|')
  });
  query.r = ~new Date;
  query._ = +new Date;
  return this.request('https://webpush.weixin.qq.com/cgi-bin/mmwebwx-bin/synccheck', { 
    query,
    headers: { Cookie: this.cookie(session) },
  })
  .then(res => res.text())
  .then(text => {
    var d = text.split('=');
    if(d.length == 2){
      var v = d[1];
      v = v.replace(/(['"])?([a-zA-Z0-9_]+)(['"])?:/g, '"$2": ');
      return JSON.parse(v);
    }
  });
};

WeChatClient.prototype.loop = function(session, { User, SyncKey }){
  const self = this;
  (function loop(){
    self
      .check(session, SyncKey)
      .then(({retcode, selector}) => {
        switch(parseInt(retcode, 10)){
          case 0:
            break;
          case 1100:
            console.error('sign out', retcode);
            break;
          case 1101:
            console.log('login to another device');
            break;
          default:
            console.error('sync check failed', retcode);
            break;
        }
        switch (parseInt(selector, 10)) {
          case 0:
            // nothing
            break;
          case 6:
            self.emit('message:response', selector);
            break;
          case 2:
            // new message
            self.sync(session, SyncKey).then(result => {
              setTimeout(loop, 1000);
              SyncKey = result.SyncKey;
              result.AddMsgList
              .filter(msg => msg.MsgType === 1)
              .forEach(msg => console.log(msg.FromUserName, msg.Content))
            });
            break;
          case 7:
            // session active
            break;
          default:
            console.error('unknow selector', selector);
            break;
        }
      });
  })();
  return User;
};

WeChatClient.prototype.sync = function(session, SyncKey){
  return this.request(`${WeChatClient.API_LOGIN}/cgi-bin/mmwebwx-bin/webwxsync`, {
    method: 'post',
    headers: { Cookie: this.cookie(session) },
    body: {
      BaseRequest: this.BaseRequest(session), SyncKey
    }
  }).then(res => res.json())
};

/**
 * [exports description]
 * @type {[type]}
 */
module.exports = WeChatClient;
//   /**
//    * [BaseRequest description]
//    */
//   get BaseRequest(){
//     return {
//       Uin      : this.uin   ,
//       Sid      : this.sid   ,
//       Skey     : '' ,
//       DeviceID : this.deviceId
//     };
//   }
//   /**
//    * [init description]
//    * @param  {Function} callback [description]
//    * @return {[type]}            [description]
//    */
//   init(){
//     var self = this;
//     return new R()
//     .post(this.api + '/cgi-bin/mmwebwx-bin/webwxinit')
//     .cookie(this.options)
//     .send({ BaseRequest: this.BaseRequest })
//     .end().then(function(res){
//       var d = JSON.parse(res.text);
//       self.Data    = d;
//       self.User    = d.User;
//       self.SyncKey = d.SyncKey;
//       self.ChatSet = d.ChatSet;
//       self.emit(WeChatClient.EVENTS.READY, d);
//       return d;
//     });
//   }
//   /**
//    * [getContacts description]
//    * @return {[type]} [description]
//    */
//   contacts(){
//     var self = this;
//     return new R()
//     .get(this.api + '/cgi-bin/mmwebwx-bin/webwxgetcontact')
//     .cookie(this.options)
//     .end().then(function(res){
//       var d = JSON.parse(res.text);
//       self.emit(WeChatClient.EVENTS.CONTACTS, d);
//       return d;
//     });
//   }
//
//
//   /**
//    * [send description]
//    * @param  {[type]} msg [description]
//    * @param  {[type]} to  [description]
//    * @return {[type]}     [description]
//    */
//   send(msg, to){
//     return new R()
//     .post(this.api + '/cgi-bin/mmwebwx-bin/webwxsendmsg')
//     .cookie(this.options)
//     .send({
//       Msg : {
//         Type        : 1,
//         Content     : msg,
//         FromUserName: this.User.UserName,
//         ToUserName  : to || this.User.UserName,
//         LocalID     : ~~new Date,
//         ClientMsgId : ~~new Date
//       },
//       BaseRequest: this.BaseRequest
//     })
//     .end().then(function(res){
//       return JSON.parse(res.text);
//     });
//   }
//   /**
//    * [sync description]
//    * @return {[type]} [description]
//    */

// };

//   contacts(){
//     var self = this;
//     return this.client.contacts().then(function(contacts){
//       self.Contacts = contacts.MemberList;
//       self.emit('contacts', self.Contacts);
//     });
//   }
//   parseGroupChatMessage(msg){
//     msg.Content = msg.Content.replace(/^(@[a-zA-Z0-9]+):<br\/>/, function(_, sender){
//       msg.ActualSender = sender;
//       return '';
//     });
//     return msg;
//   }
//   getUserFromUserName(username){
//     if(!this.Contacts) return;
//     if(WeChat.SuperClient.isGroupChat(username)){
//       // return this.ChatSet.
//       return;
//     }
//     return this.Contacts.filter(function(contact){
//       return contact.UserName == username;
//     })[0];
//   }
//   processMessage(msgs){
//     var self = this;
//     msgs.forEach(function(msg){
//
//       msg = self.parseGroupChatMessage(msg);
//
//       var msgType = Object.keys(WeChat.Client.MSG_TYPE).filter(function(type){
//         return WeChat.Client.MSG_TYPE[ type ] == msg.MsgType;
//       })[0];
//
//       switch(parseInt(msg.MsgType, 10)){
//         case WeChat.Client.MSG_TYPE.TEXT:
//           self.emit('message:text', msg);
//           break;
//         case WeChat.Client.MSG_TYPE.STATUS_NOTIFY:
//           self.processStatusNotify(msg);
//           break;
//         default:
//           console.error('unknow message type', msg.MsgType);
//           break;
//       }
//
//     });
//   }
//   processStatusNotify(msg){
//     switch(parseInt(msg.StatusNotifyCode, 10)){
//       case WeChat.Client.STATUS_NOTIFY_CODE.SYNC_CONV:
//         console.log(msg.StatusNotifyUserName.split(','));
//         break;
//       case WeChat.Client.STATUS_NOTIFY_CODE.ENTER_SESSION:
//         this.emit('session:enter', msg);
//         break;
//       case WeChat.Client.STATUS_NOTIFY_CODE.QUIT_SESSION:
//         this.emit('session:quit', msg);
//         break;
//       case WeChat.Client.STATUS_NOTIFY_CODE.READED:
//         this.emit('message:readed', msg);
//         break;
//       case WeChat.Client.STATUS_NOTIFY_CODE.MOMENTS:
//         this.emit('moments', msg);
//         break;
//       default:
//         console.log(msg.StatusNotifyCode);
//         break;
//     }
//   }
//
//   isGroupChat(user) {
//     var name = user.UserName || user;
//     return name && /^@@|@chatroom$/.test(name);
//   }
//
// }
