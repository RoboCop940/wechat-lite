...
WeChat.prototype.requestToken = function(){
  return this.token().then(token => token.access_token);
};
 ...
 * @return {[type]}        [description]
 */
WeChat.prototype.request = function(url, params = {}){
  let { query, body } = params;
  const urlObj = URI.parse(url, true);
  delete urlObj.href;
  delete urlObj.search;
  Object.assign(params, urlObj, params);
  query = Object.assign(urlObj.query, query);
  params.path += '?' + Object
    .keys(query)
    .filter(key => query[key] !== undefined)
    .map(key => [ key, encodeURIComponent(query[key]) ].join('=')).join('&');
  if(body && typeof body === 'object'){
    body = JSON.stringify(body);
    params.headers = params.headers || {};
    params.headers['Content-Type'] = 'application/json; charset=utf-8';
    params.headers['Content-Length'] = new Buffer(body).length;
  }
  if(params.unescape) params.path = unescape(params.path);
  return new Promise(function(resolve, reject){
    const client = require(params.protocol.slice(0, -1));
    const req = client.request(params, response => {
      ...
      .on('end', () => {
        ...
        response.text = () => response.blob().toString();
...
WeChat.prototype.ticket = function(token){
  return Promise
    ...
    .then(access_token => {
      return this.request(WeChat.API_CORE + '/ticket/getticket', {
...
WeChat.prototype.callback_ip = function() {
  return this.requestToken()
    .then(access_token => {
      return this.request(WeChat.API_CORE + '/getcallbackip', {
        query: { access_token }
      }).then(res => res.json());
    })
...
WeChat.prototype.user = function(openId, language){
  return this.requestToken().then(access_token => {
    return this.request(WeChat.API_CORE + '/user/info', {
      query: {
        access_token,
        openid: openId,
        lang  : language || 'en'
...
WeChat.prototype.user_remark = function(openId, remark) {
  return this.requestToken().then(access_token => {
    return this.request(WeChat.API_CORE + '/user/info/updateremark', {
      method: 'post',
      query: { access_token },
      body: {
        openid: openId,
...
WeChat.prototype.users = function(next){
  return this.requestToken().then(access_token => {
    return this.request(WeChat.API_CORE + '/user/get', {
      query: {
        access_token,
        next_openid: next
      }
...
WeChat.prototype.users_info = function(list, language){
  ...
  return this.requestToken().then(access_token => {
    return this.request(WeChat.API_CORE + '/user/info/batchget', {
      method: 'post',
      query: { access_token },
      body: { user_list: list }
    }).then(res => res.json());
...
WeChat.prototype.menu_list = function(token){
  return this.requestToken().then(access_token => {
    return this.request(WeChat.API_CORE + '/get_current_selfmenu_info', {
      query: { access_token }
    }).then(res => res.json());
  });
};
...
WeChat.prototype.template_send = function(templateId, data, url, to){
  ...
  return this.requestToken().then(access_token => {
    return this.request(WeChat.API_CORE + '/message/template/send', {
      method: 'post',
      query: { access_token },
      body: {
        touser      : to,
...
WeChat.prototype.jscode2session = function(appId, secret, code, grant_type){
  return this.requestToken().then(access_token => {
    return this.request(WeChat.API_SNS + '/jscode2session', {
      query: {
        access_token,
        appid     : appId ,
        secret    : secret,
...
WeChat.prototype.custom_send = function(openId, msgType, content){
  ...
  return this.requestToken().then(access_token => {
    return this.request(WeChat.API_CORE + '/message/custom/send', {
      method: 'post', body
    }).then(res => res.json());
  })
};
...
WeChat.prototype.qr = function(action, info, expire){
  return this.requestToken().then(access_token => {
    return this.request(WeChat.API_CORE + '/qrcode/create', {
      method: 'post',
      query: { access_token },
      body: {
        action_name: action,
...
WeChat.prototype.short_url = function(url, action){
  ...
  return this.requestToken().then(access_token => {
    return this.request(WeChat.API_CORE + '/shorturl', {
      method: 'post',
      query: { access_token },
      body: {
        action  : action,
