...
class WeChat extends EventEmitter {
  ...
  token(grant_type = 'client_credential') {
    ...
    return xttp(WeChat.API_CORE + '/token').query({
      appid,
      secret,
      grant_type
    });
...
WeChat.prototype.auth_check_token = function (openid) {
  return xttp(WeChat.API_SNS + '/auth').query({ openid });
};
...
WeChat.prototype.auth_user = function (token, openId, language) {
  return xttp(WeChat.API_SNS + '/userinfo').query({
    access_token: token,
    openid: openId,
    lang: language || 'en'
  });
...
WeChat.prototype.user_remark = function (openid, remark) {
  return this.requestToken().then(access_token => {
    return xttp(WeChat.API_CORE + '/user/info/updateremark', {
      method: 'post',
      query: {
        access_token
      },
 ...
 */
WeChat.prototype.users = function (next_openid) {
  return this.requestToken().then(access_token => {
 ...
 */
WeChat.prototype.menu_list = function () {
  return this.requestToken().then(access_token => {
...
WeChat.prototype.jscode2session = function (appId, secret, code, grant_type) {
  return this.requestToken().then(access_token => {
    return xttp(WeChat.API_SNS + '/jscode2session', {
      query: {
        access_token,
        appid: appId,
        secret: secret,
...
WeChat.prototype.custom_send = function (openId, msgType, content) {
  ...
  return this.requestToken().then(access_token => {
    return xttp(WeChat.API_CORE + '/message/custom/send', {
      method: 'post',
      body
    }).then(res => res.json());
  })
